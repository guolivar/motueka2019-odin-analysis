...
# Load ALL ODIN names and IDs
all.odins <- read_delim("./all_odin_devices.csv",
...
sites=unique(sd$deviceid)
# It is in fact 10 minutes
t5mins= seq(ISOdatetime(2019,4,1,0,10,0, tz="UTC"), ISOdatetime(2019,9,15,0,0,0,tz="UTC"), by=(60*10)) 

test2=data.frame(site=numeric(),date=character(),pm10=numeric())
...
patched.data <- foreach(j=1:nsites,
                        ...
                        .errorhandling = 'remove') %dopar%
                        {
                          ...
                          sizetel <- length(dftel$deviceid)
                          ###do these to 5min
                          dfsd=subset(sd, (deviceid)==i)
                          sizesd <- length(dfsd$deviceid)
                          ###do these to 5min
                          
                          # If both subsets are empty, then we don't have any data and we should return and move on
                          if ((sizetel+sizesd)==0){
                            # Return an empty dataframe
                            test2
                          } else {
                            # There is some data so start the matching and patching
                            dftel$DeviceTime5min <- floor_date(dftel$date, "10 mins")
                            dftel_5min <- dftel %>% group_by(DeviceTime5min) %>% summarize(mean(PM10))
                            dftel_5min<- as.data.frame(dftel_5min)
                            
                            dfsd$DeviceTime5min <- floor_date(dfsd$datetimecor, "10 mins")
                            dfsd_5min <- dfsd %>% group_by(DeviceTime5min) %>% summarize(mean(PM10))
...
save(patched.data.PM10, file = './patched.data.PM10.RData')
print("Done")
